...
const REQUIRED_FIELDS: Array<keyof AotaLead> = [
  "companyName",
  "contactName",
  "email",
  "phone",
...
function renderEmail(lead: AotaLead): { html: string; text: string } {
  ...
  const html = `
    <div style="font-family:'DM Sans',Arial,sans-serif;max-width:640px;margin:0 auto;padding:24px;color:#191919;">
      <h1 style="font-size:22px;margin:0 0 4px;">New AOTA conference lead</h1>
      <p style="color:#666;margin:0 0 20px;font-size:14px;">Submitted via /aota</p>
...
export async function POST(request: NextRequest) {
  ...
  let lead: AotaLead;
  try {
    lead = (await request.json()) as AotaLead;
  } catch {
  ...
  const subjectName = lead.companyName?.trim() || lead.contactName?.trim() || "lead";
  try {
    const { error } = await resend.emails.send({
      from: fromEmail,
      to: toEmail,
      replyTo: lead.email,
      subject: `AOTA conference lead — ${subjectName}`,
      html,
      text,
    });
    if (error) {
      console.error("[aota] resend error", error);
      return NextResponse.json(
    ...
    console.error("[aota] resend exception", err);
    return NextResponse.json(
      { error: "Could not send email. Please try again." },
      { status: 502 }
    );

import { NextRequest, NextResponse } from "next/server";
import { Resend } from "resend";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/* ─── Rate limiting ─────────────────────────────────────────────────────── */

const RATE_LIMIT_WINDOW_MS = 60_000;
const RATE_LIMIT_MAX = 5;
const buckets = new Map<string, { count: number; windowStart: number }>();

function rateLimit(ip: string): boolean {
  const now = Date.now();
  const bucket = buckets.get(ip);
  if (!bucket || now - bucket.windowStart > RATE_LIMIT_WINDOW_MS) {
    buckets.set(ip, { count: 1, windowStart: now });
    return true;
  }
  bucket.count += 1;
  return bucket.count <= RATE_LIMIT_MAX;
}

/* ─── Payload shape ─────────────────────────────────────────────────────── */

type SupportRequest = {
  name?: string;
  organization?: string;
  email?: string;
  serialNumbers?: string;
  phone?: string;
  description?: string;
  source?: string;
};

const REQUIRED_FIELDS: Array<keyof SupportRequest> = [
  "name",
  "organization",
  "email",
  "description",
];

/* ─── Email rendering ───────────────────────────────────────────────────── */

function escapeHtml(value: string): string {
  return value
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#39;");
}

function row(label: string, value?: string): string {
  if (!value || !value.trim()) return "";
  return `<tr><td style="padding:6px 12px 6px 0;color:#666;font-weight:600;vertical-align:top;white-space:nowrap;">${escapeHtml(
    label
  )}</td><td style="padding:6px 0;color:#191919;white-space:pre-wrap;">${escapeHtml(value)}</td></tr>`;
}

function renderEmail(req: SupportRequest): { html: string; text: string } {
  const html = `
    <div style="font-family:'DM Sans',Arial,sans-serif;max-width:640px;margin:0 auto;padding:24px;color:#191919;">
      <h1 style="font-size:22px;margin:0 0 4px;">New support request</h1>
      <p style="color:#666;margin:0 0 20px;font-size:14px;">Submitted via ${escapeHtml(
        req.source || "/support"
      )}</p>

      <table style="width:100%;border-collapse:collapse;font-size:14px;">
        ${row("Name", req.name)}
        ${row("Organization", req.organization)}
        ${row("Email", req.email)}
        ${row("Phone", req.phone)}
        ${row("Serial number(s)", req.serialNumbers)}
        ${row("Description", req.description)}
      </table>
    </div>
  `;

  const text = [
    "New support request",
    "",
    `Source:        ${req.source || "/support"}`,
    `Name:          ${req.name ?? ""}`,
    `Organization:  ${req.organization ?? ""}`,
    `Email:         ${req.email ?? ""}`,
    `Phone:         ${req.phone ?? ""}`,
    `Serial(s):     ${req.serialNumbers ?? ""}`,
    "",
    "DESCRIPTION",
    req.description || "(none)",
  ].join("\n");

  return { html, text };
}

/* ─── Handler ───────────────────────────────────────────────────────────── */

export async function POST(request: NextRequest) {
  const apiKey = process.env.RESEND_API_KEY;
  const fromEmail = "Able Care <hello@able-care.co>";
  const toEmail = process.env.SUPPORT_INBOX_EMAIL || "hello@able-care.co";

  if (!apiKey) {
    return NextResponse.json(
      { error: "Email service not configured." },
      { status: 500 }
    );
  }

  const ip =
    request.headers.get("x-forwarded-for")?.split(",")[0]?.trim() ||
    request.headers.get("x-real-ip") ||
    "unknown";
  if (!rateLimit(ip)) {
    return NextResponse.json(
      { error: "Too many submissions, try again shortly." },
      { status: 429 }
    );
  }

  let payload: SupportRequest;
  try {
    payload = (await request.json()) as SupportRequest;
  } catch {
    return NextResponse.json({ error: "Invalid request body." }, { status: 400 });
  }

  for (const field of REQUIRED_FIELDS) {
    if (!payload[field] || !String(payload[field]).trim()) {
      return NextResponse.json(
        { error: `Missing required field: ${field}` },
        { status: 400 }
      );
    }
  }

  const { html, text } = renderEmail(payload);
  const resend = new Resend(apiKey);
  const subjectName = payload.organization?.trim() || payload.name?.trim() || payload.email || "support request";

  try {
    const { error } = await resend.emails.send({
      from: fromEmail,
      to: toEmail,
      replyTo: payload.email,
      subject: `Able Care support request — ${subjectName}`,
      html,
      text,
    });
    if (error) {
      console.error("[support] resend error", error);
      return NextResponse.json(
        { error: "Could not send email. Please try again." },
        { status: 502 }
      );
    }
  } catch (err) {
    console.error("[support] resend exception", err);
    return NextResponse.json(
      { error: "Could not send email. Please try again." },
      { status: 502 }
    );
  }

  return NextResponse.json({ ok: true });
}

...
async function getPageData(slug: string) {
  try {
    ...
    const filePath = path.join(process.cwd(), `content/solutions/${slug}.json`);
...
export default async function SolutionPage({
    ...
    return (
      <div className="min-h-screen flex items-center justify-center pt-24">
        <div className="text-center">
          ...
          <p className="text-ac-black/60">Solution page content coming soon.</p>
        </div>
      </div>
    );
  }
  ...
  const blocks = data.blocks.map((block: any) => ({
    ...block,
    __typename: `SolutionsBlocks${block._template.charAt(0).toUpperCase() + block._template.slice(1)}`,
  }));
